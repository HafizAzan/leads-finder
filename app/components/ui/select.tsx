...
function Select({
  ...
  const onTriggerKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>) => {
    ...
    if (event.key === "ArrowDown") {
      event.preventDefault();
      setActiveIndex((prev) => (prev >= enabledOptions.length - 1 ? 0 : prev + 1));
    }
    ...
    <div className="w-full space-y-1.5" ref={rootRef}>
      ...
      <div className="relative">
        ...
        >
          ...
          <ChevronDown
            className={`size-4 shrink-0 text-muted transition-transform duration-150 ${open ? "rotate-180 text-purple-300" : ""}`}
          />
        </button>
