...
import { EmailQueueItem, Lead } from "@/types/lead";
...
function Home() {
  ...
  useEffect(() => {
    if (!progressMessage?.includes("Google Places failed")) return;
    const timer = window.setTimeout(() => setProgressMessage(null), 5000);
    return () => window.clearTimeout(timer);
  }, [progressMessage]);

  const selectedLeads = useMemo(() => leads.filter((lead) => selectedRows.includes(lead.id)), [leads, selectedRows]);
  const filteredData = useMemo(() => leads.filter((lead) => searchLeadMatches(lead, searchValue)), [leads, searchValue]);
  ...
  const selectedApprovable = selectedLeads.filter(
    (lead) => lead.aiReview.status === "approved" && lead.outreach.approval === "pending" && lead.outreach.subject,
  );
  ...
  const generateEmails = async () => {
    ...
    setGlobalBusy(true);
    setProgressMessage(`Generating emails for ${selectedLeads.length} lead(s)...`);
    markBusy(selectedLeads.map((lead) => lead.id), true);
    setError(null);

    try {
      await apiSend<{ results: Array<{ leadId: string; success: boolean; error?: string }> }>(
        "/api/ai/generate-emails",
        "POST",
        { leadIds: selectedLeads.map((lead) => lead.id) },
      );
      await loadLeads();
      setProgressMessage("Email generation complete.");
    } catch (err) {
      setError(err instanceof ApiClientError ? err.message : "Failed to generate emails.");
    } finally {
      markBusy(selectedLeads.map((lead) => lead.id), false);
      setGlobalBusy(false);
      setTimeout(() => setProgressMessage(null), 1500);
    }
  };
  ...
  const fixLeadById = async (id: string) => {
    ...
    setError(null);
    try {
      const lead = await apiSend<Lead>("/api/ai/fix-email", "POST", { leadId: id });
      upsertLead(lead);
    } catch (err) {
      setError(err instanceof ApiClientError ? err.message : "Failed to fix email.");
    } finally {
      markBusy([id], false);
  ...
  const fixAllWarnings = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.message : "Failed to fix emails.");
    } finally {
      setGlobalBusy(false);
      setProgressMessage(null);
    }
  ...
  const approveAllSelected = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.message : "Failed to approve leads.");
    }
  };
  ...
  const queueApprovedEmails = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.message : "Failed to queue emails.");
    } finally {
      setGlobalBusy(false);
      setProgressMessage(null);
    }
  ...
  const onDeleteConfirm = async () => {
    ...
    try {
      ...
      if (reviewLeadId && deleteIds.includes(reviewLeadId)) setReviewLeadId(null);
      setDeleteOpen(false);
      setDeleteIds([]);
    } catch (err) {
      setError(err instanceof ApiClientError ? err.message : "Failed to delete leads.");
    }
  };
  ...
  const tableButtons: TableButtons[] =
    selectedRows.length > 0
      ? [
          { label: `${selectedRows.length} Leads Selected`, shortLabel: `${selectedRows.length}` },
          {
    ...
    <section className="px-3 py-3 sm:px-4 sm:py-4">
      ...
      {isListEmpty ? (
        ...
        <>
          ...
          <Pagination currentPage={page} totalPages={totalPage} onPageChange={(next) => {
            setPage(next);
            setSelectedRows([]);
          }} />
        </>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={() => void onDeleteConfirm()} className="flex-1 bg-red-600 border-red-600 sm:flex-none">
              Delete
            </Button>
          </>
        }
      ...
      <EmailQueueModal open={queueOpen} items={queueItems} message={queueMessage} onClose={() => setQueueOpen(false)} />
    </section>
  );
}
